 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "## Objective: Maximum Transparency & Zero-Pipeline Architecture\n",
    "\n",
    "**Project Architecture:**\n",
    "- **Base Models (10):** NB, LogReg, SVM, RF, MLP, TextCNN, LSTM, Bi-LSTM, Bi-GRU, DistilBERT.\n",
    "- **Stacking Logic:** 5-Fold Stratified Cross-Validation (Out-of-Fold).\n",
    "- **Feature Matrix:** 30 Model Features + 7 Scaled Metadata Features.\n",
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": ["## PART 2: DATA LOADING & EXPLICIT CLEANING"]
  },
  {
   "cell_type": "code",
   "execution_count": None,
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": ["## PART 3: EXPLICIT FEATURE PREPARATION"]
  },
  {
   "cell_type": "code",
   "execution_count": None,
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": ["## PART 4: MODEL HELPER FUNCTIONS"]
  },
  {
   "cell_type": "code",
   "execution_count": None,
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": ["## PART 5: THE 10-MODEL EXPLICIT OOF LOOP"]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   ...
   "source": [
    ...
    "        v_probs = []\n",
    "        for txt in v_txts:\n",
    "            tokenized = bert_t(txt, return_tensors='pt', padding=True, truncation=True, max_length=128).to(DEVICE)\n",
    "            logits = bert_m(**tokenized).logits\n",
    "            probs = torch.softmax(logits, dim=-1).cpu().numpy()\n",
    "            v_probs.append(probs[0])\n",
    "        oof_train[v_idx, 27:30] = np.array(v_probs)\n",
    "        # Inference Test\n",
    ...
    "            logits = bert_m(**tokenized).logits\n",
    "            probs = torch.softmax(logits, dim=-1).cpu().numpy()\n",
    "            t_probs.append(probs[0])\n",
    "        oof_test[:, 27:30] += (np.array(t_probs) / 5)\n",
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": ["## PART 6: THE STACKING MATRIX & META-LEARNER"]
  },
  {
   "cell_type": "code",
   "execution_count": None,
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": ["## PART 7: FINAL EVALUATION"]
  },
  {
   "cell_type": "code",
   "execution_count": None,
